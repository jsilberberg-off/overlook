        ...
        <div className="pt-2 border-t border-white/40">
          <div className="flex items-center justify-between gap-4">
            <div>
              ...
              <div className="text-xs text-slate-500 mt-1">Uses your configured endpoint; should not add facts you did not provide.</div>
            </div>
            <button
              onClick={requestAI}
          ...
          {ai.error && <div className="mt-3 text-xs text-rose-700 bg-rose-50 border border-rose-100 px-4 py-3 rounded-2xl">{ai.error}</div>}
          ...
          {ai.result && (
            <div className="mt-4 space-y-4">
              ...
              {!ai.result.improvements && !ai.result.risks && (
                <pre className="text-xs bg-slate-950 text-slate-100 rounded-2xl p-4 overflow-x-auto">{JSON.stringify(ai.result, null, 2)}</pre>
              )}
            </div>
          )}
        </div>
